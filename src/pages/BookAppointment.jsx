...
import { CheckCircle, Clock, ArrowLeft, CalendarDays, RefreshCw, Copy, Check as CheckIcon } from "lucide-react";
...
export default function BookAppointment() {
  ...
  if (submitted) {
    const dmMessage = `Hi! I just booked an appointment with Véloire Hair:\n\nName: ${formData.client_name}\nService: ${chosenService?.name || ""}\nDate: ${format(selectedDate, "d MMMM yyyy")}\nTime: ${selectedTime}\n\nLooking forward to it!`;
    return <ConfirmationScreen formData={formData} chosenService={chosenService} selectedDate={selectedDate} selectedTime={selectedTime} dmMessage={dmMessage} />;
  }
      ...
      <div className="max-w-3xl mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="text-center mb-12">
          ...
          <p className="text-muted-foreground max-w-md mx-auto">
            Choose your service, pick a date and time, and fill in your details. We'll confirm via Instagram DM.
          </p>
        </motion.div>
...
function ConfirmationScreen({ formData, chosenService, selectedDate, selectedTime, dmMessage }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center pt-20 pb-24 px-6">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center max-w-lg w-full">
        ...
        <h2 className="font-heading text-3xl font-semibold text-foreground mb-2">Booking Request Sent!</h2>
        <p className="text-muted-foreground mb-6">Thank you, {formData.client_name}. Your slot has been reserved.</p>
        ...
        <div className="bg-card border border-border rounded-xl p-4 mb-4 text-left">
          ...
          <button onClick={handleCopy}
            className={`mt-3 flex items-center gap-2 text-sm font-medium px-4 py-2 rounded-full transition-all w-full justify-center border ${
              copied ? "bg-green-950/40 border-green-700/40 text-green-400" : "bg-secondary border-border text-foreground hover:bg-secondary/70"
            }`}>
        ...
        <a href="https://www.instagram.com/veloire_hair/" target="_blank" rel="noopener noreferrer"
          ...
          Open @veloire_hair on Instagram
        </a>
        <p className="text-xs text-muted-foreground mt-3">Copy the message above, then paste it in the DM</p>
      </motion.div>
    </div>
  );
}
